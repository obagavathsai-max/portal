          ...
          {rows.map((row, ri) => (
            <tr key={ri}>
              {row.map((cell, ci) => {
                ...
                const todayStyles = isToday(cell) ? 'font-bold bg-aums-yellow-highlight border border-aums-yellow-border' : '';
                const weekendStyles = !isToday(cell) && (isFri || isSat) ? 'text-aums-teal' : '';
                const regularStyles = !isToday(cell) && !isFri && !isSat ? 'text-gray-800' : '';

                return (
                  <td key={ci} className="text-center py-1">
                    {cell !== null ? (
                      <span
                        className={`inline-flex items-center justify-center w-6 h-6 rounded-sm text-xs cursor-pointer ${todayStyles} ${weekendStyles} ${regularStyles}`}
                      >
                        {cell}
                      </span>
                    ) : null}
